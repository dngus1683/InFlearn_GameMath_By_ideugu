...
void SoftRenderer::LoadScene3D()
{
	...
	goPlayer.GetTransform().SetPosition(Vector3::Zero);								// 플레이어의 최초 위치는 (0, 0, 0)
	goPlayer.GetTransform().SetScale(Vector3::One * playerScale);
	goPlayer.GetTransform().SetRotation(Rotator(0.f, 0.f, 0.f));					// 플레이어는 회전을 부여x
	goPlayer.SetColor(LinearColor::Blue);

	// 카메라 설정
	CameraObject& mainCamera = g.GetMainCamera();
	mainCamera.GetTransform().SetPosition(Vector3(0.f, 0.f, 500.f));				// 카메라의 최초 위치는 (0, 0, 180)
	mainCamera.GetTransform().SetRotation(Rotator(180.f, 0.f, 0.f));				// 카메라는 z축의 양의 방향에 있으며, 해당 엔진에서는 z축이 모니터 들어가는 방향이기 때문에
																					// 플레이어는 카메라 뒤에 있다.
																					// 때문에 카메라를 180도 돌려 카메라를 향하게 한다.
}
...
void SoftRenderer::Update3D(float InDeltaSeconds)
{
	...
	goPlayer.GetTransform().AddPosition(Vector3::UnitZ * input.GetAxis(InputAxis::ZAxis) * moveSpeed * InDeltaSeconds);		// 입력을 통해 플레이어가 z축으로 이동하도록 함 == 카메라 화면 상 앞뒤로만 움직임.
	goPlayer.GetTransform().AddPitchRotation(-input.GetAxis(InputAxis::WAxis) * rotateSpeed * InDeltaSeconds);				// 입력을 통해 플레이어가 피치 회전 하도록 함 == 앞으로 숙임.

	// 입력에 따른 카메라 트랜스폼의 변경
	camera.GetTransform().AddYawRotation(-input.GetAxis(InputAxis::XAxis) * rotateSpeed * InDeltaSeconds);					// 입력을 통해 카메라가 요 회전 하도록 함 == 카메라가 앞으로 숙임.
	camera.GetTransform().AddPitchRotation(-input.GetAxis(InputAxis::YAxis) * rotateSpeed * InDeltaSeconds);				// 입력을 통해 카메라가 피치 회전 하도록 함 == 카메라가 좌우로 회전.
}
...
void SoftRenderer::Render3D()
{
	...
	// 렌더링 로직의 로컬 변수
	const Matrix4x4 vMatrix = mainCamera.GetViewMatrix();						// 카메라 레퍼런스를 통해 뷰 행렬을 가져옴.
	...
	for (auto it = g.SceneBegin(); it != g.SceneEnd(); ++it)
	{
		...
		Matrix4x4 finalMatrix = vMatrix * transform.GetModelingMatrix();		// 뷰 행렬과 모델링 행렬을 곱해 최종 행렬 계산.
		...
		{
			Vector3 viewPosition = vMatrix * transform.GetPosition();			// 뷰 공간으로 변환된 좌표 계산.
