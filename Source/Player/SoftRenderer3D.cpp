...
void SoftRenderer::Update3D(float InDeltaSeconds)
{
	...
	// ************************************************************** 짐벌락 체험(스페이스 바를 누르면 강제로 피치 회전 -90도로 진행하여 두 축의 회전을 동일하게) *********************************
	if (input.IsPressed(InputButton::Space))
	{
		r.Pitch = -90.f;															// 스페이스바를 누르면 피치 회전이 -90도가 되도록 강제 설정.
	}
	else
	{
		r.Pitch += input.GetAxis(InputAxis::ZAxis) * rotateSpeed * InDeltaSeconds;	// 스페이스바를 누르지 않으면 z축 버튼을 통해 피치 회전 제어.
	}
	r.Roll += input.GetAxis(InputAxis::XAxis) * rotateSpeed * InDeltaSeconds;		// x축 버튼을 통해 롤 회전 제어.
	r.Yaw += input.GetAxis(InputAxis::YAxis) * rotateSpeed * InDeltaSeconds;		// y축 버튼을 통해 요 회전 제어.
